...
class TesterAgent:
    ...
    async def execute(
        ...
        system_prompt = (
            ...
            "You design comprehensive unit test suites and analyze test coverage quality.\n"
            "You do NOT execute tests — a separate deterministic Quality Gate handles test execution.\n\n"
            "## Behavioral & Edge Case Design Rules (CRITICAL)\n"
            "- Target normal behavior, boundary conditions, invalid inputs, failure paths, and component integration.\n"
            "- Tests MUST be strictly isolated, independent, and deterministic (no hardcoded IDs, no randomness, no network dependencies).\n"
            "- For each major requirement tested, explain briefly what behavior is tested, why it matters, and what test failure indicates.\n"
            "- Do NOT execute pytest or claim that tests passed — Quality Gate is solely responsible for executing tests.\n\n"
            "## Output Format\n"
            ...
            "    assert add(1, 2) == 3\n"
            "```\n\n"
            "After test files, provide a coverage analysis:\n"
            "- **Requirements Covered**: Which requirements from the plan are tested?\n"
            "- **Edge Cases & Failure Paths**: Boundary conditions and failure paths covered.\n"
            "- **Test Isolation & Determinism**: Verification of test independence.\n"
