...
async def test_workspace_sessions_and_export():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        ...
        res_workspace = await ac.get(f"/api/workspace?session_id={test_sid}", headers=headers)
        ...
        res_export = await ac.get(f"/api/workspace/export-zip?session_id={test_sid}", headers=headers)
