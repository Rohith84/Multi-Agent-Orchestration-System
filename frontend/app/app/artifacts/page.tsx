  ...
  return (
    <main className="flex-1 p-6 sm:p-8 lg:p-12 max-w-7xl mx-auto w-full">
      ...
      <div className="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <Link href="/" className="inline-flex items-center gap-2 text-sm text-zinc-400 hover:text-white transition-colors mb-3">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-zinc-400">Live UI Sandbox • Vision Diagram Analysis • Side-by-Side Diff • Artifact Versioning</p>
            </div>
          </div>
        </div>
            ...
            {artifacts.length === 0 ? (
              ...
              <div className="space-y-2 max-h-[600px] overflow-y-auto pr-1">
                {artifacts.map((art) => {
                  ...
                  return (
                    <div key={art.id} onClick={() => handleSelectArtifact(art)}
                      className={`p-3 rounded-lg border backdrop-blur-sm cursor-pointer transition-all ${isSelected ? "bg-zinc-800/80 border-violet-500/40 ring-1 ring-violet-500/30" : "bg-zinc-900/40 border-zinc-800 hover:border-zinc-700"}`}>
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-bold text-white truncate max-w-[160px]">{art.title}</span>
                        <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-violet-500/20 text-violet-300">v{art.current_version}</span>
                      </div>
                      <p className="text-[10px] text-zinc-500 mt-1 capitalize">Type: {art.artifact_type} • Agent: {art.creator_agent}</p>
                    </div>
                  );
                })}
              </div>
              ...
              <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-4 space-y-2 backdrop-blur-sm">
                ...
                <div className="space-y-1">
                  {selectedArtifact.versions.map((v) => (
                    <button key={v.id} onClick={() => setSelectedVersionNum(v.version)}
                      className={`w-full text-left px-2.5 py-1.5 rounded text-xs flex items-center justify-between ${selectedVersionNum === v.version ? "bg-violet-600 text-white font-bold" : "text-zinc-400 hover:bg-zinc-800 hover:text-white"}`}>
                      <span>v{v.version} — {v.change_summary}</span>
                      {selectedVersionNum !== v.version && (
                        <RotateCcw className="h-3 w-3 text-zinc-500 hover:text-white shrink-0" onClick={(e) => { e.stopPropagation(); handleRestoreVersion(v.version); }} />
                      )}
                    </button>
                  ))}
                </div>
                {selectedArtifact.versions.length >= 2 && (
                  <Button onClick={handleFetchDiff} disabled={actionLoading} variant="outline" size="sm" className="w-full mt-2 border-zinc-700 text-zinc-300 text-xs">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-zinc-800 pb-3">
              <div className="flex gap-2">
                {[{ id: "preview", label: "Live Sandbox Preview", icon: Eye },
                  ...
                  { id: "vision", label: "Vision Analysis", icon: ImageIcon }].map((tab) => {
                    const Icon = tab.icon;
                    const isActive = activeTab === tab.id;
                    return (
                      <button key={tab.id} onClick={() => setActiveTab(tab.id as StudioTab)}
                        className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${isActive ? "bg-violet-500/20 text-violet-300 border border-violet-500/30" : "text-zinc-400 hover:text-white hover:bg-zinc-800/40"}`}>
              ...
              {activeTab === "preview" && (
                <div className="flex items-center gap-1 bg-zinc-900 border border-zinc-800 p-1 rounded-lg">
                  <button onClick={() => setViewport("desktop")} className={`p-1.5 rounded ${viewport === "desktop" ? "bg-zinc-800 text-white" : "text-zinc-400"}`}>
                    <Monitor className="h-3.5 w-3.5" />
                  </button>
                  <button onClick={() => setViewport("tablet")} className={`p-1.5 rounded ${viewport === "tablet" ? "bg-zinc-800 text-white" : "text-zinc-400"}`}>
                    <Tablet className="h-3.5 w-3.5" />
                  </button>
                  <button onClick={() => setViewport("mobile")} className={`p-1.5 rounded ${viewport === "mobile" ? "bg-zinc-800 text-white" : "text-zinc-400"}`}>
            ...
            {activeTab === "preview" && (
              <div className="flex flex-col items-center">
                <div className={`transition-all duration-300 border border-zinc-800 rounded-xl overflow-hidden bg-white shadow-2xl ${getViewportWidthClass()}`}>
                  <iframe srcDoc={getCurrentContent() || "<h2 style='font-family:sans-serif;padding:2rem;text-align:center;'>No UI component preview available</h2>"}
                    className="w-full h-[550px] border-0" title="Live Sandbox Preview" sandbox="allow-scripts" />
                </div>
              </div>
            )}
            {/* TAB 2: SOURCE CODE */}
            ...
            {activeTab === "diff" && (
              <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-5 space-y-3 backdrop-blur-sm">
                ...
                {diffData ? (
                  ...
                  <div className="p-8 text-center text-xs text-zinc-500">Click "Compare Versions" in the sidebar to generate a version diff.</div>
                )}
              </div>
            )}
            ...
            {activeTab === "vision" && (
              <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-5 space-y-4 backdrop-blur-sm">
                ...
                {visionResult ? (
                  ...
                  <div className="p-8 text-center text-xs text-zinc-500">Upload an image or UI sketch using the button above to view vision analysis results.</div>
                )}
              </div>
            )}
          </div>
